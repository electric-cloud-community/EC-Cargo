...
sub main() {
	...
	if ($containerLocation eq CargoCommon->REMOTE) {
		print "Only local containers can be stopped";
		$::gEC->setProperty("/myJobStep/outcome", 'error');		
	} else {	
		#if the container is "tomcat6x"
		if ($container eq CargoCommon->TOMCAT6X_TYPE) {
			Tomcat6xWrapper->stopContainer($filePath, $container, $containerLocation, $containerPath, $user, $pass, $url);
			sleep $::gSleep;		
			Tomcat6xWrapper->verifyContainer($container, $user, $pass, $url, "stop");
		} elsif ($container eq CargoCommon->JETTY8X_TYPE) {
			print "Jetty stop container not supported";
			$::gEC->setProperty("/myJobStep/outcome", 'error');		
		} elsif ($container eq CargoCommon->JBOSS7X_TYPE) {
			JBoss7xWrapper->stopContainer($filePath, $container, $containerLocation, $containerPath, $user, $pass, $url);
			sleep $::gSleep;		
			JBoss7xWrapper->verifyContainer($container, $user, $pass, $url, "stop");
		} elsif ($container eq CargoCommon->WEBLOGIC10X_TYPE) {
			WebLogic103xWrapper->stopContainer($filePath, $container, $containerLocation, $containerPath, $user, $pass, $url);
			sleep $::gSleep;		
			WebLogic103xWrapper->verifyContainer($container, $user, $pass, $url, "stop");
		}
	}	
}
