...
########################################################################
# deployTomcat - runs the command to make the deploy
#
# Arguments:
...
sub deployApp {	
	...
	#execute command
	my $cmd = "mvn cargo:deploy -f \"$filePath\" -P $container";
	...
	#load the configuration template of tomcat to add to the pom.xml file
	my $configTemplate = CargoCommon->TOMCAT6X_CONFIG;
	...
	if ($containerLocation eq CargoCommon->LOCAL) {
		...
		$configTemplate =~ s/<cargo.remote.uri>_url_<\/cargo.remote.uri>//g;			
		...
		$configTemplate =~ s/<cargo.remote.uri>_url_<\/cargo.remote.uri>/<cargo.remote.uri>$url\/manager<\/cargo.remote.uri>/g;	
	}
	...
	#preplace in the template the tomcat container values
	...
	$configTemplate =~ s/<cargo.remote.username>_tomcatUser_<\/cargo.remote.username>/<cargo.remote.username>$user<\/cargo.remote.username>/g;
	$configTemplate =~ s/<cargo.remote.password>_tomcatPass_<\/cargo.remote.password>/<cargo.remote.password>$pass<\/cargo.remote.password>/g;
	
	#replace in the pom.xml file the "tomcat6x" container value for the modified template
	foreach my $node ($profileNodeset->get_nodelist) {
		my $nodeStr = XML::XPath::XMLParser::as_string($node);   
	
		if ($nodeStr =~ m/<id>tomcat6x<\/id>/) {				
			if ($strFile =~ m/<id>tomcat6x<\/id>(.*)<\/build>/s) {
				$strFile =~ s/<id>tomcat6x<\/id>(.*)<\/build>/$configTemplate/s;
			}
		} 		
...
########################################################################
# undeployTomcat - runs the command to make the undeploy
#
# Arguments:
#   -propHash: hash containing path of the pom.xml file, the pon.xml as a xml object, the container, the location of the container, the user, password and url of the config
#
...
sub undeployApp {	
	...
	#execute command
	my $cmd = "mvn cargo:undeploy -f \"$filePath\" -P $container";
...
sub verifyContainer {	
	...
	my $urlTomcat = $url . "/cargocpc";
	my $justRun = $::gDeployApp;
	
    #create all objects needed for response-request operations
    my $agent = LWP::UserAgent->new(env_proxy => 1,keep_alive => 1, timeout => 30);
    my $header = HTTP::Request->new(GET => $urlTomcat);
    my $request = HTTP::Request->new('GET', $urlTomcat, $header);
...
sub runContainer {		
	...
	if ($containerLocation eq CargoCommon->LOCAL) {
		...
		$configTemplate =~ s/<type>_configType_<\/type>/<type>runtime<\/type>/g;
		$configTemplate =~ s/<home>_selectedContainer_<\/home>//g;	
		$configTemplate =~ s/<cargo.remote.uri>_url_<\/cargo.remote.uri>/<cargo.remote.uri>$url<\/cargo.remote.uri>/g;	
	}
	...
	foreach my $node ($profileNodeset->get_nodelist) {
		...
		if ($nodeStr =~ m/<id>tomcat6x<\/id>/) {				
			if ($strFile =~ m/<id>tomcat6x<\/id>(.*)<\/build>/s) {
				$strFile =~ s/<id>tomcat6x<\/id>(.*)<\/build>/$configTemplate/so;
			} 
		}
		open DATA, ">$filePath" or die "can't open $filePath $!";
		printf DATA $strFile;
