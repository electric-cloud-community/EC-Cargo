...
########################################################################
# deployJetty - runs the command to make the deploy
#
# Arguments:
...
sub deployApp {	
	...
	#load the configuration template of jetty to add to the pom.xml file
	my $configTemplate = CargoCommon->JETTY8X_CONFIG;
	...
	if ($containerLocation eq CargoCommon->LOCAL) {
		...
		$configTemplate =~ s/<type>_configType_<\/type>/<type>standalone<\/type>/g;
	...
	#preplace in the template the jetty container values
	...
	$configTemplate =~ s/<cargo.hostname>_jettyHost_<\/cargo.hostname>/<cargo.hostname>$host<\/cargo.hostname>/g;	
	$configTemplate =~ s/<cargo.servlet.port>_jettyPort_<\/cargo.servlet.port>/<cargo.servlet.port>$port<\/cargo.servlet.port>/g;	
	$configTemplate =~ s/<cargo.remote.username>_jettyUser_<\/cargo.remote.username>/<cargo.remote.username>$user<\/cargo.remote.username>/g;
	$configTemplate =~ s/<cargo.remote.password>_jettyPass_<\/cargo.remote.password>/<cargo.remote.password>$pass<\/cargo.remote.password>/g;
	
	#replace in the pom.xml file the "jetty8x" container value for the modified template
	foreach my $node ($profileNodeset->get_nodelist) {
		my $nodeStr = XML::XPath::XMLParser::as_string($node);   
	
		if ($nodeStr =~ m/<id>jetty8x<\/id>/) {				
			if ($strFile =~ m/<id>jetty8x<\/id>(.*)<\/build>/s) {
				$strFile =~ s/<id>jetty8x<\/id>(.*)<\/build>/$configTemplate/s;
			}
		} 		
...
########################################################################
# undeployJetty - runs the command to make the undeploy
#
# Arguments:
#   -propHash: hash containing path of the pom.xml file, the pon.xml as a xml object, the container, the location of the container, the user, password and url of the config
#
...
sub verifyContainer {	
    ...
    my $header = HTTP::Request->new(GET => $urlJetty);
    my $request = HTTP::Request->new('GET', $urlJetty, $header);
