...
########################################################################
# deployWebLogic - runs the command to make the deploy
#
# Arguments:
...
sub deployApp {	
	...
	#execute command
	my $cmd = "mvn cargo:deploy -f \"$filePath\" -P $container -Dorg.weblogic.as.client.connect.timeout=50000";
	...
	#load the configuration template of weblogic to add to the pom.xml file
	my $configTemplate = CargoCommon->WEBLOGIC10X_CONFIG;
	...
	if ($containerLocation eq CargoCommon->LOCAL) {
		...
		$configTemplate =~ s/<home2>_selectedContainer_<\/home2>/<home>$containerPath<\/home>/g;
		$configTemplate =~ s/<cargo.weblogic.server>_weblogicHost_<\/cargo.weblogic.server>/<cargo.weblogic.server>$host<\/cargo.weblogic.server>/g;	
		$configTemplate =~ s/<cargo.servlet.port>_weblogicPort_<\/cargo.servlet.port>/<cargo.servlet.port>$port<\/cargo.servlet.port>/g;	
	...
	#preplace in the template the weblogic container values
	...
	$configTemplate =~ s/<cargo.weblogic.administrator.user>_weblogicUser_<\/cargo.weblogic.administrator.user>/<cargo.weblogic.administrator.user>$user<\/cargo.weblogic.administrator.user>/g;
	$configTemplate =~ s/<cargo.weblogic.administrator.password>_weblogicPass_<\/cargo.weblogic.administrator.password>/<cargo.weblogic.administrator.password>$pass<\/cargo.weblogic.administrator.password>/g;
	
	#replace in the pom.xml file the "weblogic7x" container value for the modified template
	foreach my $node ($profileNodeset->get_nodelist) {
		my $nodeStr = XML::XPath::XMLParser::as_string($node);   
	
		if ($nodeStr =~ m/<id>weblogic103x<\/id>/) {				
			if ($strFile =~ m/<id>weblogic103x<\/id>(.*)<\/build>/s) {
				$strFile =~ s/<id>weblogic103x<\/id>(.*)<\/build>/$configTemplate/s;
			}
		} 		
...
########################################################################
# undeployWebLogic - runs the command to make the undeploy
#
# Arguments:
#   -propHash: hash containing path of the pom.xml file, the pon.xml as a xml object, the container, the location of the container, the user, password and url of the config
#
...
sub undeployApp {	
	...
	#execute command
	my $cmd = "mvn cargo:undeploy -f \"$filePath\" -P $container -Dorg.weblogic.as.client.connect.timeout=50000";
...
sub verifyContainer {	
	...
	my $urlWebLogic = $url . "/console/App.html#deployment-list";
	
    #create all objects needed for response-request operations
    my $agent = LWP::UserAgent->new(env_proxy => 1,keep_alive => 1, timeout => 30);
    my $header = HTTP::Request->new(GET => $urlWebLogic);
    my $request = HTTP::Request->new('GET', $urlWebLogic, $header);
...
sub runContainer {		
	...
	#if container is local, use local types and remove the url to use the home
	if ($containerLocation eq CargoCommon->LOCAL) {
		my $home2 = $containerPath;
		$home2 =~ s/(.*)\\user_projects(.*)/$1\\wlserver_10\.3/;
		$configTemplate =~ s/<type>_deployType_<\/type>/<type>installed<\/type>/g;
		$configTemplate =~ s/<type>_configType_<\/type>/<type>existing<\/type>/g;
		$configTemplate =~ s/<home>_selectedContainer_<\/home>/<home>$containerPath<\/home>/g;
		$configTemplate =~ s/<home2>_selectedContainer_<\/home2>/<home>$home2<\/home>/g;
		$configTemplate =~ s/<cargo.weblogic.server>_weblogicHost_<\/cargo.weblogic.server>/<cargo.weblogic.server>$host<\/cargo.weblogic.server>/g;	
		$configTemplate =~ s/<cargo.servlet.port>_weblogicPort_<\/cargo.servlet.port>/<cargo.servlet.port>$port<\/cargo.servlet.port>/g;	
...
sub stopContainer {		
	...
	if ($containerLocation eq CargoCommon->LOCAL) {
		my $home2 = $containerPath;
		$home2 =~ s/(.*)user_projects(.*)/$1wlserver_10\.3/;
		$configTemplate =~ s/<home2>_selectedContainer_<\/home2>/<home>$home2<\/home>/g;
		$configTemplate =~ s/<type>_deployType_<\/type>/<type>installed<\/type>/g;
		$configTemplate =~ s/<type>_configType_<\/type>/<type>existing<\/type>/g;
		$configTemplate =~ s/<home>_selectedContainer_<\/home>/<home>$containerPath<\/home>/g;
